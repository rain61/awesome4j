...
**运行示例代码观察输出及结合扩容源码可以发现：**  

1. ArrayList初始容量为10  

2. 当向ArrayList中添加元素时，会根据需要动态扩容，即：如果ArrayList.elementData剩余可存储空间不足以存储待添加元素时将会触发扩容，扩容大小至少是当前ArrayList.elementData的0.5倍(为什么是至少？当调用addAll()方法时大有可能出现扩容0.5倍仍不足以存储添加进来的新元素)。